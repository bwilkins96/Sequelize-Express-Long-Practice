...
router.post('/', async (req, res, next) => {
    try {
        ...
        let newTree = await Tree.create({tree: name, location: location, heightFt: height, groundCircumferenceFt: size});
...
router.delete('/:id', async (req, res, next) => {
    try {
        let treeRmv = await Tree.findByPk(req.params.id);
        await treeRmv.destroy();
...
router.put('/:id', async (req, res, next) => {
    try {
        ...
        if (tree) {
            if (name) {await tree.update({ tree: name })}
            if (name) {await tree.update({ location: location })}
            if (name) {await tree.update({ heightFt: height })}
            if (name) {await tree.update({ groundCircumferenceFt: size })}
